...
class SupplierHomeViewModel extends ChangeNotifier {
  ...
  SupplierHomeViewModel(this._store) {
  ...
  int get totalOrders =>
      completedOrders.length + _user.totalOrders;
  ...
  Order createListing({
    ...
    final orderId = 'SUP-MKT-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
    final order = Order(
      id: orderId,
  ...
  void addOrder(Order order) {
    // Order already in store via MarketplaceViewModel.purchaseItem.
    // Just switch to orders tab.
    _currentTab = 2;
    notifyListeners();
  }
