...
RETURNS TEXT LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT email FROM public.users WHERE phone = phone_number LIMIT 1;
$$;
...
BEGIN
  IF NEW.driver_id IS NOT NULL AND NEW.status = 'accepted' THEN
    SELECT COUNT(*) INTO active_count FROM public.orders
      WHERE driver_id = NEW.driver_id AND status IN ('accepted', 'inTransit') AND id <> NEW.id;
...
CREATE POLICY users_select_participant ON public.users FOR SELECT USING (
  id IN (
...
-- ── Storage buckets ────────────────────────────────────────────
-- Run these in Dashboard → Storage (SQL cannot create buckets directly)
-- OR use the Supabase client SDK. Documented here for reference:
--
--   bucket: profile-photos   | public: true  | max: 5 MB  | types: image/jpeg, image/png, image/webp
--   bucket: user-documents   | public: false | max: 10 MB | types: image/jpeg, image/png, image/webp, application/pdf
--   bucket: proof-photos     | public: false | max: null  | types: null (all)
--
-- Paste into SQL Editor after creating buckets via Dashboard:
